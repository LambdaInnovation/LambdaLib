...
public abstract class BlockMulti extends BlockContainer {
    ...
    public AxisAlignedBB getRenderBB(int x, int y, int z, ForgeDirection dir) {
        ...
        return AxisAlignedBB.getBoundingBox(box.minX + x, box.minY + y, box.minZ + z, box.maxX + x, box.maxY + y,
                box.maxZ + z);
    }
    ...
    // Rotation API
    // Some lookup tables
    private static final ForgeDirection[] rotMap = { 
        NORTH, // -Z,
        EAST, // +X,
        SOUTH, // +Z,
        WEST // -X
    ...
    public TileEntity getOriginTile(TileEntity now) {
        ...
        TileEntity ret = validate(
                now.getWorldObj().getTileEntity(now.xCoord - sbp.dx, now.yCoord - sbp.dy, now.zCoord - sbp.dz));
        return ret;
    }
