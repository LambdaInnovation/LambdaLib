 ...
 */
package cn.liutils.vis.editor.animation;
...
import cn.liutils.vis.curve.IFittedCurve.Point;
import cn.liutils.vis.editor.common.VEVars;
import cn.liutils.vis.editor.common.widget.Window;
import cn.liutils.vis.editor.modifier.RealModifier;
...
public class CurveView extends Window {
	...
	public double precision = 0.03;
	
	public CurveView(IFittedCurve _curve) {
		super("Curve Viewer");
		curve = _curve;
		
		transform.setSize(300, 160);
	...
	private void initViewArea() {
		...
		area.transform.setPos(0, 10).setSize(270, 110);
	...
	private void initWidgets() {
		double x = 40, s = 65;
		doubleModifier(x, -10, "l", "min{x}");
		doubleModifier(x += s, -10, "r", "max{x}");
		doubleModifier(x += s, -10, "d", "min{y}");
		doubleModifier(x += s, -10, "u", "max{y}");
	}
	...
	private void doubleModifier(double x, double y, String fieldName, String desc) {
		try {
			...
			LIUtils.log.error("Init CurveView", e);
		}
	}
